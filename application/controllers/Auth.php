...
class Auth extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->data['keterangan_aplikasi']= $sekolah->pr_ket_aplikasi;
		$this->data['nama_sekolah']= $sekolah->pr_nama;
		$this->data['alamat_sekolah']= $sekolah->pr_alamat;
		$this->data['telepon_sekolah']= $sekolah->pr_telepon;
	...
	public function index()
	{
		...
		else if ($this->session->userdata('level')=='admin_group') // remove this elseif if you want to enable this for non-admins
		{
			// redirect them to the home page because they must be an administrator to view this
			redirect('administrator/dashboard');
		} 
		else if ($this->session->userdata('level')=='guru_group') // remove this elseif if you want to enable this for non-admins
		{
			// redirect them to the home page because they must be an administrator to view this
			redirect('guru/dashboard');
		} 
		else if ($this->session->userdata('level')=='walikelas_group') // remove this elseif if you want to enable this for non-admins
		{
			// redirect them to the home page because they must be an administrator to view this
		...
		{
			show_error('Anda tidak memiliki akses, silahkan login dengan akun yang ada !');
		}
	}
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $this->input->post('level'), $this->input->post('tahun'), $remember))
			{
				...
				if ($level=='admin_group'){
					$g = 'admin';
				} else if ($level=='guru_group'){
					$g = 'guru';
				} else if ($level=='walikelas_group'){
					$g = 'walikelas';
				}
				$check = $this->Model_users->check_group($idusers_log,$g);
				if($idusers_log!='' && $check==true){
					$log_activity = [
						'idtahun_pelajaran' => $this->input->post('tahun'),
						'idusers' => $idusers_log,
					...
					$log_activity = [
						'idtahun_pelajaran' => $this->input->post('tahun'),
						'idusers' => $idusers_log,
						'activity' => 'gagal login karena salah memilih level user sebagai '.$g,
						'log_date' => date('Y-m-d H:i:s')
					];
					$this->Model_activity->create_data($log_activity);
				}
			...
			else
			{
				...
				if($idusers_log!=''){
					...
					$check = $this->Model_activity->read_data_by_identity($this->input->post('identity'));
					if($check>0){
						$log_activity = [
							'idtahun_pelajaran' => $this->input->post('tahun'),
							'idusers' => $check,
		...
		else
		{
			...
			$this->data['level'] = [
				...
				'guru_group' => 'Guru',
				'walikelas_group' => 'Guru Kelas',
			];
			
			$this->data['level_attribut'] = array(
				'name'			=>'level',
				'id'			=>'level',
				'class'			=>'form-control',
				'autocomplete'	=> 'on',
	...
	public function forgot_password()
	{
		...
		else
		{
			...
			if ($forgotten)
			{
                ...
                $this->email->from('robots@ghalyfadhillah.com','SIPS | LUPA PASSWORD');
                ...
                if ($this->email->send()) {
					...
					if ($this->input->post('type_identity')=='administrator'){
						$r['status'] = "ok";
					} else {
