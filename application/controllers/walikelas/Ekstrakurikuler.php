...
class Ekstrakurikuler extends CI_Controller
{
    ...
    public function save() {
        ...
        if ($data['_mode'] == 'add'){
            ...
            $this->log_activity($this->session->userdata('nama').' menambah data nilai ekstrakurikuler siswa ');
            $r['status'] = "ok";
        } else if ($data['_mode'] == 'edit'){
            $this->Model_ekstra_siswa->update_data($insert,$id);
            $this->log_activity($this->session->userdata('nama').' mengubah data nilai ekstrakurikuler siswa ');
            $r['status'] = "ok";
        } else {
            $r['status'] = "gagal";
        }
    ...
    public function delete($id,$nama) {
        ...
        $this->log_activity($this->session->userdata('nama').' menghapus data nilai ekstrakurikuler siswa '.filter(str_replace('%20',' ',$nama)));
        $r['status'] = "ok";
        header('Content-Type: application/json');
        echo json_encode($r);
	}
