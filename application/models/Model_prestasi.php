...
{
    protected $table = 'sr_prestasi';
	protected $id = 'idprestasi';
	protected $order = 'DESC';

	var $column = array('idprestasi','k_tingkat','s_nama','p_jenis','p_keterangan','idprestasi');

	function _get_datatables_query()
	{
        $this->db->join('sr_tahun_pelajaran','sr_tahun_pelajaran.idtahun_pelajaran = sr_prestasi.idtahun_pelajaran','left');
        $this->db->join('sr_users','sr_users.idusers = sr_prestasi.idusers','left');
        $this->db->join('sr_siswa','sr_siswa.idsiswa = sr_prestasi.idsiswa','left');
	...
	public function get_datatables($tahun,$id)
	{
        ...
        $this->db->where('sr_prestasi.idtahun_pelajaran',$tahun);
	...
	public function read_data_by_siswa($id,$tahun,$idusers)
    {
		$this->db->select('*','edit as mode');
        $this->db->join('sr_siswa','sr_siswa.idsiswa = sr_prestasi.idsiswa','left');
        $this->db->where('sr_prestasi.idsiswa',$id);
        $this->db->where('sr_prestasi.idtahun_pelajaran',$tahun);
        $this->db->where('sr_prestasi.idusers',$idusers);
