...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$this->session->set_userdata('insert_user_id',$id);
	 ...
	 * @author    Mathew
	 */
	public function login($identity, $password, $level, $tahun, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, first_name, last_name, phone')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				$this->set_session($user,$level,$tahun);
	 ...
	 * @author jrmadsen67
	 */
	public function set_session($user,$level,$tahun)
	{
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, remember_code, last_login, first_name, last_name')
