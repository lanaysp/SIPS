      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col-12">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    ...
                    <input type="hidden" name="_nilai_sosial" id="_nilai_sosial" value="">
                    <input type="hidden" name="_nilai_sosial_meningkat" id="_nilai_sosial_meningkat" value="">
                    <input type="hidden" name="_idnilai_sosial" id="_idnilai_sosial" value="">
            ...
            <!-- /.card -->
            <div class="card" id="divSosial" style="display:none;">
              <!-- /.card-header -->
              <div class="card-body table-responsive">
                <div class="float-right"><small><i>* jika kolom input sikap kosong / tidak ada berarti anda belum melakukan rencana penilaian sikap sosial</i></small></div>
                ...
                <table id="tSosial" class="table table-bordered table-striped">
...
$(document).delegate("#nilai_sosial_meningkat","change",function( ) {
  ...
  var status = 'meningkat';
    $.ajax({
        ...
        url: base_url+"Nilai_sosial/update_nilai",
    });
});
...
$(document).ready(function(){
  ...
  $("#idkelas").on("change", function( ) {
    var idkelas = $('#idkelas').val();
    $('#divSosial').fadeIn();
    fDatatables("tSosial","<?=base_url('guru/Nilai_sosial/ajax_list/')?>"+idkelas,"ASC");
    fDuplicate("tSosial","nth-child(2)");
    $.ajax({
          type: "GET",
          url: base_url+"Nilai_sosial/jumlah_nilai/"+idkelas,
  ...
  $("#f_siswa").on("submit", function(e) {
    ...
    if (idkelas==''){
      ...
      $.ajax({
          ...
          url: base_url+"Nilai_sosial/add_one_siswa",
          success: function(r) {
              if (r.status=='ok'){
                $('#f_siswa')[0].reset();
  ...
  $("#btn-save").on("click", function( ) {
    ...
    var nilai_sosial = $('select[name="nilai_sosial[]"]').map(function(){return $(this).val();}).get();
    var idnilai_sosial = $('select[name="nilai_sosial[]"]').map(function(){return $(this).attr('data-id');}).get();
    var status = 'rutin';
    var p = 0;
    var q = 0;
    var semua_nilai = '';
    var idnilai_sosial_new = '';
    var total_data = jumlah_nilai * jumlah_data;

    for (i=0;i<total_data;i++) {
      p++;
      var index = (total_data + p) - (total_data);
      $("#_nilai_sosial").val(nilai_sosial[index - 1]);
      var current_nilai = $("#_nilai_sosial").val();
      semua_nilai = semua_nilai + current_nilai + ',';
      $("#_nilai_sosial").val(semua_nilai);

      $("#_idnilai_sosial").val(idnilai_sosial[index - 1]);
      idnilai_sosial_new = $("#_idnilai_sosial").val();

      if(((semua_nilai.split(',').length) - 1) == jumlah_nilai){
        $.ajax({
            type: "POST",
            data: {semua_nilai:semua_nilai,idnilai_sosial_new:idnilai_sosial_new,status:status},
            url: base_url+"Nilai_sosial/update_nilai",
        });
        $("#_nilai_sosial").val('');
        $("#_idnilai_sosial").val('');
...
function reset_data() {
  ...
  if (confirm('Apakah anda yakin ingin mereset seluruh data..? ** PENTING ** Seluruh data penilaian akan direset menjadi 0')) {
    $.ajax({
    type: "GET",
    url: base_url+"Nilai_sosial/reset_data/"+idkelas,
    success: function(r) {
      if (r.status == 'ok'){
