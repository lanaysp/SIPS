          ...
          <div class="col-12">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    ...
                    <input type="hidden" name="_idrencana_bs_sosial" id="_idrencana_bs_sosial" value="">
            ...
            <div class="card" id="divSosial" style="display:none;">
              ...
              <div class="card-body table-responsive">
                <div class="float-right"><small><i>* jika kotak yang telah dicentang tidak tersimpan setelah klik tombol simpan silahkan refresh halaman kemudian coba kembali</i></small></div>
...
$(document).ready(function(){
  $("#idkelas").on("change", function( ) {
    ...
    $.ajax({
        ...
        url: base_url+"Rencana_sosial/check_penilaian",
        success: function(r) {
            if (r.status == "y") {
              $('#jumlah_ph').val(r.data.rbs_so);
              $('#_idrencana_bs_sosial').val(r.data.idrencana_bs_sosial);
              $('#_idbutir_sikap').val(r.data.rbs_so_kd);
              $('#divSosial').fadeIn();
              fDatatables("tSosial","<?=base_url('guru/Rencana_sosial/ajax_list/')?>"+idkelas,"ASC");
            } else if (r.status == "n") {
              $('#jumlah_ph').val(r.data.rbs_so);
              $('#_idrencana_bs_sosial').val(r.data.idrencana_bs_sosial);
              $('#_idbutir_sikap').val(r.data.rbs_so_kd);
  ...
  $("#jumlah_ph").on("change", function( ) {
    ...
    if(formValidation()){
      ...
      $.ajax({
          ...
          url: base_url+"Rencana_sosial/save",
          success: function(r) {
              if (r.status == "update") {
  ...
  $("#btn-save").on("click", function( ) {
    var idrencana_bs_sosial = $("#_idrencana_bs_sosial").val();
    var idbutir_sikap = $("#_idbutir_sikap").val();
    var idkelas = $('#idkelas').val();
    $.ajax({
        type: "POST",
        data: {idbutir_sikap:idbutir_sikap,idrencana_bs_sosial:idrencana_bs_sosial},
        url: base_url+"Rencana_sosial/update_rencana_sosial",
        success: function(r) {
            if (r.status == "ok") {
                showToast('Data diupdate !',1000,'success');
                fDatatables("tSosial","<?=base_url('guru/Rencana_sosial/ajax_list/')?>"+idkelas,"ASC");
