          ...
          <div class="col-md-12">
            <div class="card card-outline" style="background-color:#2D5E89; color:#fff;">
              <div class="card-header">
                ...
                <a onclick="return edit(0)" class="btn btn-info" style="color:#FFF; float:right;">(+) Tambah Nilai</a>
              </div>
            </div>
          </div>
          <!-- /.col -->
          ...
          <div class="col-12">
            ...
            <div class="card">
              ...
              <div class="card-body table-responsive">
                <table id="tEkstra" class="table table-bordered table-striped table-hover">
              ...
              <div class="card card-primary card-tabs">
                <div class="card-header p-0 pt-1">
                  <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                    <li class="nav-item">
                      <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill" href="#custom-tabs-one-home" role="tab" aria-controls="custom-tabs-one-home" aria-selected="true">Nilai Ekstrakurikuler <span class="error-tab1"></span></a>
                    </li>
                  </ul>
                </div>
                  ...
                  <div class="tab-content" id="custom-tabs-one-tabContent">
                    <div class="tab-pane fade show active" id="custom-tabs-one-home" role="tabpanel" aria-labelledby="custom-tabs-one-home-tab">
                      <div class="card-body">
                        ...
                        <div class="form-group">
                          <label for="idekstra">Ekstrakurikuler</label>
                          <div class="error-message2"></div>
                          <?=form_dropdown('',$list_ekstra,'',$list_ekstra_attribute)?>
                        </div>
                        <div class="form-group">
                          <label for="es_nilai">Nilai</label>
                          <div class="error-message3"></div>
                          <?=form_dropdown('',$es_nilai,'',$es_nilai_attribute)?>
                        </div>
                        <div class="form-group">
                          <label for="es_deskripsi">Deskripsi / Keterangan</label>
                          <div class="error-message4"></div>
                          <input type="text" name="es_deskripsi" class="form-control" id="es_deskripsi" placeholder="Keterangan nilai ekstrakurikuler" value="" required>
                        </div>
                      </div>
                    </div>
                  </div>
...
function formValidation()
{
  ...
  var idekstra = $('#idekstra').val();
  var es_nilai = $('#es_nilai').val();
  var es_deskripsi = $('#es_deskripsi').val();
  ...
  } else if ($.trim(idekstra)==''){
    showToast('Ekstrakurikuler tidak boleh kosong !',1000,'error');
  ...
  } else if ($.trim(es_nilai)==''){
    showToast('Nilai tidak boleh kosong !',1000,'error');
...
$(document).ready(function(){
  fDatatables("tEkstra","<?=base_url('walikelas/Ekstrakurikuler/ajax_list')?>","ASC");
  ...
  $("#f_ekstra").on("submit", function(e) {
    ...
    if(formValidation()){
      ...
      $.ajax({
          type: "POST",
          data: data,
          url: base_url+"Ekstrakurikuler/save",
          success: function(r) {
              if (r.status == "gagal") {
...
function edit(id){
  ...
  $.ajax({
      type: "GET",
      url: base_url+"Ekstrakurikuler/edit/"+id,
      success: function(result) {
          $("#_id").val(result.data.idekstra_siswa);
          $("#idsiswa").val(result.data.idsiswa);
          $("#idekstra").val(result.data.idekstra);
          $("#es_nilai").val(result.data.es_nilai);
          $("#es_deskripsi").val(result.data.es_deskripsi);
...
function delete_data(id,data,ekstra) {
  if (id == 0) {
    showToast('Silahkan pilih data yang akan dihapus !',1000,'error');
  }
  swal({
      title: "Lanjut hapus ..?",
      text: "Apakah anda yakin ingin menghapus nilai ekstrakurikuler "+ekstra+" dari siswa "+data+"..? [PENTING] Data yang telah dihapus tidak dapat dikembalikan",
      icon: "warning",
      buttons: [
        'Batal',
        'Lanjut'
      ...
      if (isConfirm) {
        $.ajax({
        type: "GET",
        url: base_url+"Ekstrakurikuler/delete/"+id+"/"+data,
        success: function(r) {
          if (r.status == 'ok'){
