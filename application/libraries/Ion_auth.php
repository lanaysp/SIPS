...
class Ion_auth
{
	...
	public function is_admin($id = FALSE)
	{
		...
		$check = $this->Model_users->check_group($idusers,$g);
		if ($level=='admin_group' && $check==true){
			$admin_group = $this->config->item($level, 'ion_auth');
			return $this->ion_auth_model->in_group($admin_group, $id);
		} else {
			$this->session->set_flashdata('message', 'Anda bukan admin, silahkan login dengan data yang benar!');
		}
	}
	...
	public function is_guru($id = FALSE)
	{
		...
		$idusers = $this->session->userdata('user_id');
		$check = $this->Model_users->check_group($idusers,$g);
		if ($level=='guru_group' && $check==true){
			$guru_group = $this->config->item($level, 'ion_auth');
			return $this->ion_auth_model->in_group($guru_group, $id);
		} else {
			$this->session->set_flashdata('message', 'Anda bukan guru, silahkan login dengan data yang benar!');
		}
	}
	
	public function is_walikelas($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_walikelas');
		$level = $this->session->userdata('level');
		if ($level=='admin_group'){
			$g = 'admin';
		} else if ($level=='guru_group'){
		...
		$idusers = $this->session->userdata('user_id');
		$check = $this->Model_users->check_group($idusers,$g);
		if ($level=='walikelas_group' && $check==true){
			$walikelas_group = $this->config->item($level, 'ion_auth');
			return $this->ion_auth_model->in_group($walikelas_group, $id);
		} else {
			$this->session->set_flashdata('message', 'Anda bukan guru kelas / wali kelas, silahkan login dengan data yang benar!');
		}
	}
